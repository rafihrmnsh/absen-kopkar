...
export default function Login() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      toast({ title: "Login gagal", description: err?.message ?? "Email atau password salah" });
    } finally {
      setLoading(false);
    }
  }
  ...
  async function handleGoogleLogin() {
    ...
    try {
      ...
      toast({ title: "Login berhasil dengan Google" });
      navigate("/");
    } catch (err: any) {
      toast({ title: "Login gagal", description: err?.message ?? "Terjadi kesalahan dengan Google" });
    } finally {
      setLoading(false);
    }
  }
    ...
    <div className="min-h-[80vh] flex items-center justify-center px-4">
      <div className="w-full max-w-sm rounded-2xl border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 shadow-sm">
        ...
        <div className="px-6 pb-6 text-center text-sm text-zinc-500">
          Belum punya akun?{" "}
          <Link to="/register" className="underline text-brand hover:opacity-80">
            Daftar
          </Link>
        </div>
      </div>
